	...
	public void run() {
		...
		while (active) {
			...
			if (mCore.getConnection().isConnected()) {
				...
				try {
					// get worlddata
					String xml = mCore.getConnection().getDatagrammString(
							timeout);
					...
					if (xml != null && !xml.isEmpty()) {
						...
						WorldData wData = WorldData
								.unmarshallXMLPositionDataPackageString(xml);

						// update graphics
						int minutes = (int) (wData.getPlayTime() / 60);
						int seconds = (int) (wData.getPlayTime() - wData
								.getPlayTime() / 60);
						int scoreBlue = wData.getScore().getScoreBlueTeam();
						int scoreYellow = wData.getScore().getScoreYellowTeam();

						String title = String.format(
								"%s %02d : %02d %s - %02d:%02d", "Yellow",
								scoreYellow, scoreBlue, "Blue", minutes,
								seconds);
