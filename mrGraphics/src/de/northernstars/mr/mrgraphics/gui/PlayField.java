...
public class PlayField extends JPanel implements ComponentListener {
	...
	private WorldData mWorld = ScenarioInformation.getInstance().getWorldData();
	...
	public PlayField() {
		...
		try {
			mBallImage = ImageIO
					.read(PlayField.class
							.getResource("/de/northernstars/mr/mrgraphics/gui/ball.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}

		float dash1[] = { 10.0f };
		mDashedStroke = new BasicStroke(1.0f, BasicStroke.CAP_BUTT,
	...
	public void paintComponent(Graphics g) {
		try {
			...
			for (Player vPlayer : mWorld.getListOfPlayers()) {
				...
				g2d.drawLine(
						(int) (mWidth * vPlayer.getPosition().getX() + 0.010
								* mWidth
								* Math.cos(Math.toRadians(-vPlayer
										.getOrientationAngle()))),
						(int) (-mHeight * vPlayer.getPosition().getY() + 0.010
								* mWidth
								* Math.sin(Math.toRadians(-vPlayer
										.getOrientationAngle()))),
						(int) (mWidth * vPlayer.getPosition().getX() + 0.010
								* mWidth
								* Math.cos(Math.toRadians(-vPlayer
										.getOrientationAngle() - 90))),
						(int) (-mHeight * vPlayer.getPosition().getY() + 0.010
								* mWidth
								* Math.sin(Math.toRadians(-vPlayer
										.getOrientationAngle() - 90))));
				...
				// (int)( mWidth * vPlayer.getPosition().getX() + 0.010 * mWidth
				// * Math.cos( Math.toRadians( -vPlayer.getOrientationAngle() )
				// ) ),
				// (int)( -mHeight * vPlayer.getPosition().getY() + 0.010 *
				// mWidth * Math.sin( Math.toRadians(
				// -vPlayer.getOrientationAngle() ) ) ) );

				g2d.fillOval(
						(int) (mWidth * vPlayer.getPosition().getX() - mHalfPointSize),
						(int) (-mHeight * vPlayer.getPosition().getY() - mHalfPointSize),
						(int) (mPointSize), (int) (mPointSize));
				g2d.drawOval(
						(int) (mWidth * vPlayer.getPosition().getX() - 0.010 * mWidth),
						(int) (-mHeight * vPlayer.getPosition().getY() - 0.010 * mWidth),
						(int) (0.020 * mWidth), (int) (0.020 * mWidth));

				String vPlayerString = vPlayer.getNickname() + " ("
						+ vPlayer.getId() + ")";

				g2d.drawString(
						vPlayerString,
						(int) (mWidth * vPlayer.getPosition().getX() - g2d
								.getFontMetrics(mFont).stringWidth(
										vPlayerString) / 2),
						(int) (-mHeight * vPlayer.getPosition().getY())
								+ g2d.getFontMetrics(mFont).getHeight());

				g2d.setStroke(mDashedStroke);
				g2d.drawOval(
						(int) (mWidth * vPlayer.getPosition().getX() - 0.013 * mWidth),
						(int) (-mHeight * vPlayer.getPosition().getY() - 0.013 * mWidth),
						(int) (0.026 * mWidth), (int) (0.026 * mWidth));
			...
			g2d.drawImage(
					mBallImage,
					(int) (mWidth
							* mWorld.getBallPosition().getPosition().getX() - mHeight / 160),
					(int) (-mHeight
							* mWorld.getBallPosition().getPosition().getY() - mHeight / 160),
					(int) (mHeight / 80), (int) (mHeight / 80), this);

			// draw score
			int minutes = (int) (mWorld.getPlayTime() / 60);
			int seconds = (int) (mWorld.getPlayTime() - mWorld.getPlayTime() / 60);
			int scoreBlue = mWorld.getScore().getScoreBlueTeam();
			int scoreYellow = mWorld.getScore().getScoreYellowTeam();

			ReferencePoint yCornerTop = mWorld.getMapOfReferencePoints().get(
					ReferencePointName.YellowFieldCornerTop);
			int timestringX = (int) (mWidth * yCornerTop.getPosition().getX());
			int timestringY = (int) (-mHeight * yCornerTop.getPosition().getY())
					- g2d.getFontMetrics(mFont).getHeight();
			int scorestringY = timestringY
					- g2d.getFontMetrics(mFont).getHeight();

			g2d.setColor(Color.WHITE);

			g2d.drawString(String.format("TIME: %02d:%02d", minutes, seconds),
					timestringX, timestringY);
			g2d.drawString(String.format("SCORE: %s %02d : %02d %s", "Yellow",
					scoreYellow, scoreBlue, "Blue"), timestringX, scorestringY);
	...
	private void createBackground() {
		...
		// draw blue goal
		if (mWorld.getMapOfReferencePoints().containsKey(
				ReferencePointName.BlueGoalCornerTop)
				&& mWorld.getMapOfReferencePoints().containsKey(
						ReferencePointName.BlueGoalCornerBottom)) {
			mTop = mWorld.getMapOfReferencePoints()
					.get(ReferencePointName.BlueGoalCornerTop).getPosition();
			mBottom = mWorld.getMapOfReferencePoints()
					.get(ReferencePointName.BlueGoalCornerBottom).getPosition();
		...
		// draw yellow goal
		if (mWorld.getMapOfReferencePoints().containsKey(
				ReferencePointName.YellowGoalCornerTop)
				&& mWorld.getMapOfReferencePoints().containsKey(
						ReferencePointName.YellowGoalCornerBottom)) {
			mTop = mWorld.getMapOfReferencePoints()
					.get(ReferencePointName.YellowGoalCornerTop).getPosition();
			mBottom = mWorld.getMapOfReferencePoints()
					.get(ReferencePointName.YellowGoalCornerBottom)
					.getPosition();
		...
		for (ReferencePoint vPoint : mWorld.getReferencePoints()) {
			...
			g.fillOval(
					(int) (mWidth * vPoint.getPosition().getX() - mHalfPointSize),
					(int) (-mHeight * vPoint.getPosition().getY() - mHalfPointSize),
					(int) (mPointSize), (int) (mPointSize));

			// g.drawString(vPoint.getPointName().getShortName(), (int)( mWidth
			// * vPoint.getPosition().getX() + mHalfPointSize ), (int)( -mHeight
			// * vPoint.getPosition().getY() + 1.5 * mPointSize ) );

		}

		mLineWidth = mPointSize * 0.25f;
		g.setColor(Color.WHITE);
		g.setStroke(new BasicStroke(mLineWidth, BasicStroke.CAP_ROUND,
				BasicStroke.JOIN_BEVEL));
		int i = 0;
		while (i < mLinesToDraw.size() - 1) {

			mRefPoint1 = mWorld.getMapOfReferencePoints().get(
					mLinesToDraw.get(i++));
			mRefPoint2 = mWorld.getMapOfReferencePoints().get(
					mLinesToDraw.get(i++));
		...
		if (mWorld.getMapOfReferencePoints().containsKey(
				...
				&& mWorld.getMapOfReferencePoints().containsKey(
						ReferencePointName.BlueGoalCornerBottom)
				&& mWorld.getMapOfReferencePoints().containsKey(
						ReferencePointName.FieldCenter)) {

			mTop = mWorld.getMapOfReferencePoints()
					.get(ReferencePointName.BlueGoalCornerTop).getPosition();
			mBottom = mWorld.getMapOfReferencePoints()
					.get(ReferencePointName.BlueGoalCornerBottom).getPosition();
			mCenter = mWorld.getMapOfReferencePoints()
					.get(ReferencePointName.FieldCenter).getPosition();
			...
			g.setStroke(new BasicStroke(mLineWidth, BasicStroke.CAP_ROUND,
					BasicStroke.JOIN_BEVEL));
			g.drawArc(
					(int) (mWidth * (mCenter.getX() - mCenterCircleSize * 0.5)),
					(int) (-mHeight * mCenter.getY() - mHeight
							* mCenterCircleSize * 0.5),
					(int) (mWidth * mCenterCircleSize),
					(int) (mHeight * mCenterCircleSize), 0, 360);
	 ...
	 * 
	 * @return {@link List} of {@link ReferencePointName}. Every pair of two
	 *         points are start and end point of line.
	 */
	 ...
	 *            the mWorld to set
	 */
	public void setWorld(WorldData mWorld) {
		this.mWorld = mWorld;
	}
