...
public class PlayField extends JPanel implements ComponentListener{
	...
	public PlayField() {
		...
		try {
				mBallImage = ImageIO.read( PlayField.class.getResource( "/de/northernstars/mr/mrgraphics/gui/ball.png" ));
			} catch (IOException e) {
				e.printStackTrace();
			}
...
//						(int)( -mHeight * vPlayer.getPosition().getY() + 0.010 * mWidth * Math.sin( Math.toRadians( -vPlayer.getOrientationAngle() ) ) ) );
			...
			ReferencePoint yCornerTop = mWorld.getMapOfReferencePoints().get(ReferencePointName.YellowFieldCornerTop);
			int timestringX = (int)( mWidth * yCornerTop.getPosition().getX() );
			int timestringY = (int)( -mHeight * yCornerTop.getPosition().getY() ) - g2d.getFontMetrics(mFont).getHeight();
			int scorestringY = timestringY - g2d.getFontMetrics(mFont).getHeight();
			
			g2d.setColor( Color.WHITE );
			
			g2d.drawString(String.format("TIME: %02d:%02d", minutes, seconds),
					timestringX, timestringY);
			g2d.drawString(String.format("SCORE: %s %02d : %02d %s", "Yellow", scoreYellow, scoreBlue, "Blue"),
					timestringX, scorestringY);
